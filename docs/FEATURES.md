...
- [ ] **P1 — React to comments (👍, ❤️, 🎉, …)** — small reaction-row at the bottom of each rendered comment. GitHub's GraphQL reaction mutations or the matching `page_data` endpoint. Without this, users must leave the page to react.
- [ ] **P1 — Hunk-aware `+` visibility** — hide the `+` button on blocks whose source line falls outside any diff hunk. Needs proper hunk parsing (from `comparison.fullDiff` or by re-fetching the unified diff and walking `@@ -a,b +c,d @@` headers). Prevents the 422 from being possible at all and gives users an immediate visual cue that those blocks aren't commentable. Note: an early attempt used `diffSummary.markersMap` keys as a stand-in for valid lines — don't repeat that, `markersMap` is markers-only (typically 3–4 keys for a long file), not hunk lines. See [DEV_NOTES.md → Snapping to valid hunks](./DEV_NOTES.md#snapping-to-valid-hunks-todo).
- [ ] **P2 — Preserve scroll position after reply submit** — after submitting a reply in an expanded thread, the rich-diff view re-renders and jumps back to the top of the file, so the user has to scroll back down to see their own reply land. Capture `window.scrollY` (and ideally the thread's `getBoundingClientRect().top`) before the re-render, then restore scroll position once the new reply is inserted. Same issue likely applies to edit/delete/resolve flows — audit all post-mutation re-renders.
...
- [ ] **P2 — Comment on deleted lines (LEFT side)** — follow-up to the deleted-blocks drift fix in 1.0.3. GitHub's source-diff lets you comment on deleted lines by posting with `side: "LEFT"` against the BASE file's old line number; in rich-diff we currently render no `+` on `<del>` blocks at all. To support this we'd need to (a) fetch the BASE version of each file (in addition to the head/post-change source we already cache), (b) build a separate source index for the base and match `<del>` blocks against it to recover the OLD line number, (c) extend `postReviewCommentInternal` to accept `side: "LEFT"` and use the old line, and (d) render existing LEFT-side threads on the corresponding deleted blocks. Roughly doubles the per-file source-fetch cost and requires LEFT-side handling throughout the thread-rendering code, so deferred until there's user demand.
