...
- [x] **Thread display polish** (shipped in 1.0.1) — visual + UX upgrades to bring the inline thread experience closer to GitHub's native source-diff thread:
  ...
  - **Role badges** — `Owner` / `Member` / `Collaborator` / `Contributor` pill next to the username, mapped from GitHub's `author_association` field. Owner gets an accent-blue chip; the rest use a neutral chip.
