...
- [ ] **P1 — Prev / next *change* navigation (jump between added / removed / modified blocks)** — distinct from the existing prev/next *comment* navigation (`j` / `k` on the threads sidebar): change-nav walks the rendered DOM's `.added` / `.removed` / `<ins>` / `<del>` wrappers in document order, while comment-nav walks the `💬` badges. This is the first thing a reviewer reaches for when opening a PR for the **first** time — they want to scan only the *edits*, not re-read the kept prose around them. Proposed keys (don't collide with the existing `j` / `k` / `h` / `l` / `t` / `Shift+T` bindings, and don't reuse GitHub's own `g j` / `g k` chord): `n` / `p` for next / previous change; jump-to-first / jump-to-last under `Shift+N` / `Shift+P`. Surface the same buttons as a header pair in the threads sidebar (next to the existing thread-nav `↑` / `↓`) so they're discoverable without the keyboard; mirror them in the inline floating bar if the sidebar is collapsed. Skip kept content even if it has comments — comment-nav is the right tool for that case. **From 2026-06 user feedback**: *"When open a MD for PR for the first time, one common navigation required is jump to the next diff or previous diff so that one can comment. Maybe consider navigations for prev/next changes, and also navigations to the prev/next comments."* (The comment-nav side of that ask is already shipped via the sidebar in 1.0.1.)
...
- [ ] **P2 — Promo tile redesign** — the current tiles (`design/promo-tiles/*.png`) have two problems flagged in the 2026-06 review:
  - **Background palette doesn't match the icon.** Near-black radial gradient (RGB 18,28,48 → 6,10,22) against a solid GitHub-blue (#0969da) speech-bubble icon reads as two unrelated visual systems instead of one brand. Move to a palette that harmonises with the icon — GitHub-blue tint, or Primer canvas `#f6f8fa` so the blue icon pops as the hero.
  - **The on-tile tagline is too long for a glance read.** Today the small grey line under the "Markdown PR Comments / for GitHub" headline reads `"Inline review comments on GitHub PR rich-diff (rendered markdown)."` — 9 words at 13–30 px on a busy gradient is read-by-nobody at thumbnail size. Shorten it dramatically in the same verb-led voice as the store short description (`"Comment, reply, resolve, and collapse sections directly in…"`) — the goal is a one-second read on a 220×140 carousel tile. Candidate taglines (all ≤ 7 words) live in the header brief of [`design/promo-tiles/generate.ps1`](../design/promo-tiles/generate.ps1); pick one or cut the tagline entirely and let the icon + headline carry the tile. Use text sparingly — anything below ~24 px effective is decoration.

  Spec lives in the header comment of [`design/promo-tiles/generate.ps1`](../design/promo-tiles/generate.ps1); regenerate with `pwsh design/promo-tiles/generate.ps1` after editing.
...
- [ ] **P2 — Shorter display name for store tiles + browser toolbar** — the current name *"Markdown PR Comments for GitHub"* (32 chars including spaces) truncates in narrow store carousels, the browser toolbar tooltip, and the `chrome://extensions/` card title. Explore the *"Short — Long"* pattern many Chrome Web Store extensions use to get both a glance-readable prefix and a descriptive long name in one string: e.g. **`Markdown PR — Markdown PR Comments for GitHub`** (the user-suggested *"Markdown PR - Markdown PR Comments for GitHub"*, with an em-dash for typography). When the listing card truncates to ~25 chars, only the short prefix shows; the full name still appears on hover, on the listing detail page, and in screen readers. **Risks:** *Markdown PR* on its own is generic enough that it may compete with unrelated keyword searches on the store; the suffix is the disambiguator. Before committing, validate with a quick search-traffic check in the Chrome Web Store / Edge Partner Center analytics that the dual-name doesn't tank the existing impression count. **From 2026-06 user feedback**: *"Feel like that you can consider a shorter name 'Markdown PR' as the current name is still too long. … One way for the naming is 'Markdown PR - Markdown PR Comments for GitHub'. I saw that people do that so that a shorter name can be used, while having the longer name to make it clear the use case."* Implementation touch points if approved: `manifest.json` `name` field; both `*_SUBMISSION.md` templates under [.github/skills/rdc-publish-check/templates/](../.github/skills/rdc-publish-check/templates/); README.md and INSTALL.md page titles; promo tiles in `design/promo-tiles/`; a `Changed` entry in CHANGELOG.md mirroring the 1.2.0 rename note style.
...
- [ ] **P1 — "Reload to activate" prompt after install on an already-open PR tab** — when a user installs the extension while a PR page is already open in another tab, the content script does **not** activate on that tab until it's reloaded. Today this is silent — the user toggles back to the PR they were just trying to comment on, sees nothing happen, and has no idea a hard refresh is needed. Add a background-script listener for `chrome.runtime.onInstalled` (`reason === 'install'` or `reason === 'update'`); for every existing tab whose URL matches `https://github.com/*/pull/*`, inject a single-line top-of-page banner: *"Markdown PR Comments for GitHub was just installed / updated. Reload this tab to activate."* with an inline **Reload** button (`chrome.tabs.reload(tabId)`) and a `Dismiss` ✕. Auto-removes on next page load — there's no persistent banner state to manage. Skip the banner on tabs where the content script is already alive (the `onInstalled` → message-tab-existing-content-script handshake distinguishes fresh installs from updates of an already-running script). **From 2026-06 user feedback**: *"A tip on hard-refresh is also useful if it is installed after the PR page is loaded."* Today this is documented only in [docs/DEV_NOTES.md](./DEV_NOTES.md) (engineer-facing) — most users never see it. Until the runtime banner ships, the equivalent tip lives in [INSTALL.md → Just installed?](../INSTALL.md#just-installed) as a text-only fallback.
