...
- **TOC heading-anchor jumps work in rich-diff.** Clicking a TOC link like `[Change Log](#change-log)` on a PR's rich-diff page now scrolls to the heading. GitHub strips heading ids in the multi-file rich-diff (duplicate ids would be invalid HTML), so the browser's native fragment scroll fails silently. The extension now listens for `hashchange` (and runs once on init): when the hash has no matching DOM target, it walks every `<h1>`–`<h6>` in the rich-diff prose body, slug-matches textContent against the hash, and scrolls to the first match — scoped to the file matching `?file=<path>` when present. New `slugifyHeading` pure helper in `src/lib/anchors.js` with 10 unit tests.
