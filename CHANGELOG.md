...
- **Avatars and role badges in threads.** Every comment shows the author's avatar and an `Owner` / `Member` / `Collaborator` / `Contributor` badge next to the username — the same context you get in GitHub's native source-diff thread view.
...
- **Dark mode polish.** The sidebar, thread cards, and comment badges now use proper dark colors on dark-mode pages — no more bright bar against the dark background.
