...
- [x] **134 unit tests** for pure helpers (line matching, response parsing, markersMap parsing, table arithmetic, markdown preview, HTML escaping, time formatting, sidebar helpers, heading slugify) using Node's built-in `node:test` — zero dependencies
...
- [ ] **P2 — Keyboard navigation between threads** — *folds into the [Threads sidebar](#) v1.* `g j` / `g k` chord (or `[` / `]`) drives the same prev/next as the sidebar header buttons. Respects GitHub's existing keybindings by mounting on a unique chord.
