# Microsoft Edge Add-ons submission — v1.5.0
...
- **Zip:** `rdc-1.5.0.zip` (in this folder)
...
```
🆕 What's new in v1.5.0 (2026-06-12)

• New "Changes" tab in the sidebar — jump straight to the next changed paragraph, list item, table row, code block, or heading without re-reading the unchanged prose. Use the new `◀ N/M ▶` counter in the header (or press `[` / `]`) to step; `{` / `}` jumps to first / last.
• Sidebar tab shortcuts: press `1` / `2` / `3` to switch to Threads / Outline / Changes. Auto-expands the sidebar if collapsed.
• Renamed display to "Markdown PR — Markdown PR Comments for GitHub" so the toolbar tooltip and store card show a short prefix that fits, while the full name stays visible everywhere else. Same extension, same install.
• "Render all Markdown files as rich-diff" CTA now also appears in the empty Changes pane (previously only in Threads), so users landing on a /changes page have an obvious next step from any tab.
• Fixed: thread prev/next arrows and the `N/M` counter on the Threads tab no longer accidentally walk the Changes cards on pages with few unresolved comments.
